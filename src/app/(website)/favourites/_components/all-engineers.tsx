...
/* =======================
   Helper: Get hours from localStorage
======================= */
const getAssignedEngineerHours = () => {
  const stored = localStorage.getItem("assignedEngineerHours");
  if (!stored) return [];
...
const AllEngineers = ({
  ...
  ======================= */
  useEffect(() => {
    setSelectAll(team.length > 0 && selected.length === team.length);
  }, [selected, team.length]);
  ...
  const mutation = useMutation({
    ...
    mutationFn: async (engineerIds: string[]) => {
      const assignedHours = getAssignedEngineerHours();

      const engineersPayload = engineerIds.map((id) => {
        const hourInfo = assignedHours.find(
          (item) => item.engineerId === id
        );

        return {
          engineer: id,
          allocatedHours: hourInfo?.hours ?? 0,
        };
      });
    ...
    onSuccess: (_, engineerIds) => {
      // Remove added engineers from localStorage favourites
      const saved = localStorage.getItem("favouriteEngineers");
      if (saved) {
        try {
          const favs: EngineerDatatype[] = JSON.parse(saved);
      ...
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-hidden flex flex-col p-0">
        <DialogHeader className="p-6 border-b">
          <DialogTitle className="text-2xl font-bold flex items-center gap-2">
            <Users className="w-6 h-6 text-[#00383B]" />
            Select Engineers
          </DialogTitle>
        </DialogHeader>
              ...
              {team.map((member) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => toggleSelect(member._id)}
                    className={cn(
                      "relative flex items-center gap-4 p-4 mb-3 rounded-xl border cursor-pointer transition",
                      isSelected
                        ? "border-[#00383B] bg-[#00383B]/5"
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">
                        ${member.rate}/hr • {member.skills?.join(", ")}
                      </p>
                    </div>

                    {isSelected && (
                      <CheckCircle2 className="text-[#00383B]" />
                    )}
        ...
        <div className="border-t px-6 py-4 flex justify-between">
          ...
          <Button
            onClick={handleSubmit}
            disabled={selected.length === 0 || mutation.isPending}
            className="bg-[#00383B] text-white"
          >
            {mutation.isPending
              ? "Adding..."
              : `Add ${selected.length} Engineer${
