...
// import { ApprovedEngineer, Project } from "./projects-data-type";
...
//           {
//             manager?.manager && member?.isManager ? "Manager" : "Make Manager"
//           }

//           {
//             manager?.manager && !member?.isManager && "Make Manager"
//           }
...
export function TeamMember({
  ...
  member: ApprovedEngineer;
  projectId: string;
  manager: Project; // contains approvedEngineers
}) {
  const session = useSession();
  const token = (session?.data?.user as { accessToken: string })?.accessToken;
  const queryClient = useQueryClient();

  // 🔍 Check if any engineer is already manager
  const managerExists = manager?.approvedEngineers?.some(
    (item) => item.isManager
  );
  ...
  // 🔵 Label text
  const buttonLabel = member.isManager ? "Manager" : "Make Manager";

  // 🔴 Disable rule:
  // If manager exists AND this member is not manager → disable
  const disableButton = isPending || (managerExists && !member.isManager);
    ...
    <div className="flex items-center justify-between gap-3 rounded-md bg-muted/40 p-3 transition-colors duration-200 hover:bg-muted/60">
      <div className="flex items-center gap-3">
        <Avatar className="h-8 w-8">
          <AvatarImage src={member?.engineer?.profileImage || ""} />
          <AvatarFallback>
            {member?.engineer?.firstName?.slice(0, 1) || "U"}
          </AvatarFallback>
        </Avatar>
      ...
      <div>
        <button
          ...
          onClick={() => handleManager(member?.engineer?._id)}
          className={`px-4 py-1 rounded-lg text-sm flex items-center gap-2 transition ${
            member.isManager
