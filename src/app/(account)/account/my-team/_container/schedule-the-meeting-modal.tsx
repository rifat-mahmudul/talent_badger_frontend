...
const ScheduleTheMeetingModal = ({ open, onOpenChange, projectId }: ScheduleMeetingProps) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: ["schedule-meeting"],
    mutationFn: async (data: { date: string; time: string; link: string; projectId: string }) => {
      const res = await fetch(`${process?.env?.NEXT_PUBLIC_BACKEND_URL}/booking`, {
        method: "POST",
        headers: {
          "Content-type": "application/json",
      ...
      <DialogContent className="bg-white">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <FormControl>
                          <Button
                            variant="outline"
                            className={`w-full h-12 border-[1px] border-[#147575] rounded-[4px] py-2 px-4 text-black placeholder:text-[#929292] text-sm font-medium leading-[150%] ${!field.value && "text-muted-foreground"
                              }`}
                          >
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input
                        type="time"
                        className="h-12 border-[1px] border-[#147575] rounded-[4px] py-2 px-4 text-black placeholder:text-[#929292] text-sm font-medium leading-[150%]"
                        {...field}
                        placeholder="Time"
                      />
                    </FormControl>
            ...
            <button
              disabled={isPending}
              className="bg-[#147575] border-[1px] border-[#147575] rounded-[4px] h-[48px] w-full text-[#F8F9FA] text-sm font-semibold leading-[150%]"
              type="submit"
            >
              {isPending ? "Scheduling..." : "Schedule The Meeting"}
            </button>
          </form>
        </Form>
      </DialogContent>
